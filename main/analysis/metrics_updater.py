...
def update_metrics(max_age_hours: int = 25, verbose: bool = False) -> dict:
    """
    ...
    try:
        ...
        # Re-rank main characters after metric updates
        if updated_data.get('main_characters'):
            # Sort by controversy score (with trend bonuses already applied)
            main_chars = updated_data['main_characters']
            
            # Apply simple re-ranking based on updated controversy scores
            for char in main_chars:
                base_controversy = char.get('controversy', 0)
                trend = char.get('trend', 'stable')
                
                # Apply trend multipliers for ranking
                if trend == 'rising':
                    char['_ranking_score'] = base_controversy * 1.15
                elif trend == 'falling':
                    char['_ranking_score'] = base_controversy * 0.9
                elif trend == 'new':
                    char['_ranking_score'] = base_controversy * 1.1
                else:
                    char['_ranking_score'] = base_controversy
            
            # Re-sort and assign new ranks
            main_chars.sort(key=lambda x: x.get('_ranking_score', 0), reverse=True)
            for i, char in enumerate(main_chars, 1):
                char['rank'] = i
                char.pop('_ranking_score', None)  # Remove temp ranking score
            
            if verbose:
                print("🔄 Re-ranked main characters after metrics update")
...
def main():
    ...
    parser.add_argument('--max-age', type=int, default=25,
                       help='Maximum age of analysis in hours (default: 25)')
