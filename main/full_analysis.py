...
# Import our modules
from main import RatioPipeline
from shared.data_transformer import MainCharacterTransformer
from shared.utils import load_today_json, save_today_json, get_current_timestamp
...
def run_full_analysis(hours_back: int = 6, 
    ...
    try:
        ...
        pipeline = RatioPipeline(
            ...
            collection_timeout=600,  # 10 minutes max
        ...
        main_characters = transformer.transform_pipeline_results(results, previous_data)
        
        if not main_characters:
            print("⚠️  No main characters identified")
            ...
            metadata = transformer.create_metadata(results, len(main_characters))
            current_data = {
                "metadata": metadata,
        ...
        print(f"Main characters identified: {len(main_characters)}")
        
        if main_characters:
            print(f"\n🔥 Top 5 Main Characters:")
            for i, char in enumerate(main_characters[:5]):
